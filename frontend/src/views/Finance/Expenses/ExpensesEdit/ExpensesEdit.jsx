...
function ExpensesEdit(props) {
  ...
  useEffect(() => {
    dispatch(fetchTransactionsDetailAction(expensesDetailId)).then(() =>
      setLoading(false))
  ...
  const dispatchedStudentList = useSelector(
    (state) => state.student.studentsDetail && state.student.studentsDetail.data.results
  );
  ...
  const editExpense = async (formData, resetForm) => {
    try {
      const response = await axios.put(`/finances/transactions/${expensesDetailId}/`, formData);
      addToast("Расход изменен!", {
        appearance: "success",
        autoDismiss: true
      });
      resetForm()
  ...
  const onSubmit = (values, {resetForm}) => {
    ...
    editExpense(formData, resetForm);
