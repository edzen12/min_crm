...
function ExpensesDetail(props) {

  const { addToast } = useToasts();
  const expensesDetailId = props.id;
  ...
  async function deleteRequestExpense() {
    setLoading(true)
    try {
      await axios.delete(`/finances/transactions/${expensesDetailId}/`)
      setLoading(false);
  ...
  function handleDeleteExpense(e, name) {
    ...
    confirm({ description: `Удалить ${name}?` })
      .then(() => deleteRequestExpense())
      .catch(() => {
        /* ... */
      });
  }

  const expenseData = useSelector(state => state.finance.transactionsDetail && state.finance.transactionsDetail.data)
        ...
        {expenseData && (
          <Paper className={classes.mainCont}>
            <Typography variant="h4" className={classes.spaceBetween}>
              {expenseData.title}
              <div>
                <IconButton onClick={e => handleDeleteExpense(e, expenseData.title)} color="primary">
                  <DeleteIcon color="inherit" />
                </IconButton>

                <IconButton component={NavLink} to={`/transactions/update/${expensesDetailId}`} color="primary">
            ...
            <GetCreatedTime date={expenseData.created_date} />
            <Divider />
            ...
            <Box className={`mt-4  mb-4 `}>
              ...
              <Box className={"mb-3"}>
                <Typography variant="body-1">
                  {expenseData.confirmation && "Файл отсутствует!"}
                </Typography>
              </Box>
              <Box className={"col-lg-2 col-md-4 p-0 col-sm-12"}>
                <Button
                  component={Link}
                  href={expenseData.confirmation}
                  variant="contained"
                  color="primary"
