...
function Tags() {
  ...
  const handleTagDelete = (tagName, tagId) => {
    ...
    confirm({ title: `Удалить тэг, ${tagName}?`, description: '' })
      .then(() => {
        setTags(tagsCopy.filter(item => item.id !== tagId));
        axios.delete(`courses/coursestags/${tagId}`)
          .then(response => {
            addToast(`Тэг ${tagName}, удален!`, {
              appearance: "success",
              autoDismiss: true
            });
          })
  ...
  const createCourseTag = () => {
    if (tagInputValue.trim() != '') {
      setSubmitting(true);
      axios.post('courses/coursestags/', { title: tagInputValue })
        .then(response => {
          setTagInputValue('');
          setSubmitting(false);
          setOpen(false);
  ...
  const updateCourseTag = () => {
    if (tagInputValue.trim() != '') {
      setSubmitting(true);
      axios.patch(`courses/coursestags/${updatingTagId}/`, { title: tagInputValue })
        .then(response => {
  ...
  return (
    <Dashboard>
      ...
      <Grid container direction="row" justify="space-between" className="mb-4">
        {tags &&
          (tags.length === 0 ? (
            <Typography variant="h6">В системе отсутствуют тэги!</Typography>
          ) : (
              <Typography
                variant="h6"
                className="mt-2"
      ...
      <Dialog open={open} onClose={handleFormClose} aria-labelledby="form-dialog-title">
        <DialogTitle id="form-dialog-title">{modalFormType} тэг</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
